    ...
    StringSpec({
        ...
        "the gauntlet definition-coverage burn-down is pinned, mainboard and sideboard separately" {
            ...
            summary.decks.map { report ->
                ...
                listOf(
                    // `FW-TAPUNTAP` moved five rows: Quirion Ranger (Elves +1, Spy Combo +1), Harrier
                    // Strix and Snap (Mono Blue Faeries +2), Sleep of the Dead (Mono-Blue Terror +1),
                    // and Snap with Azorius Chancery (UWX Familiar +2).
                    Pinned("Elves", 9, 16, 3, 5),
                    Pinned("Gates", 9, 17, 3, 5),
                    Pinned("Grixis Affinity", 17, 22, 5, 7),
                    Pinned("GW Bogles", 18, 18, 5, 8),
                    Pinned("Jeskai Ephemerate", 16, 22, 4, 7),
                    Pinned("Jund Wildfire", 16, 22, 5, 7),
                    Pinned("Mono Blue Faeries", 8, 14, 4, 6),
                    Pinned("Mono-Blue Terror", 12, 14, 6, 6),
                    Pinned("Mono-Red Madness", 12, 12, 2, 5),
                    Pinned("Mono Red Rally", 5, 13, 2, 5),
                    Pinned("Monster Tron", 9, 21, 4, 6),
                    Pinned("Spy Combo", 9, 21, 4, 8),
                    Pinned("UWX Familiar", 14, 20, 4, 6),
                )
        }
 ...
 * `FW-TAPUNTAP` added five: Harrier Strix, Snap, Azorius Chancery, Quirion Ranger, Sleep of the Dead.
 */
private const val TOTAL_ENCODED_MAIN = 113
private const val TOTAL_MISSING_MAIN = 65

/** Distinct cards named by at least one gauntlet sideboard. */
private const val TOTAL_DISTINCT_SIDEBOARD = 48
private const val TOTAL_ENCODED_SIDEBOARD = 26
private const val TOTAL_MISSING_SIDEBOARD = 22

/**
 * The whole backlog: distinct undefined cards across both boards of all thirteen decks. `FW-TAPUNTAP`
 * took five off it — all five are mainboard cards, so [TOTAL_MISSING_SIDEBOARD_ONLY] is unmoved.
 */
private const val TOTAL_MISSING_BOTH_BOARDS = 84

/** Of the backlog, the cards that appear only in sideboards. */
private const val TOTAL_MISSING_SIDEBOARD_ONLY = 19
