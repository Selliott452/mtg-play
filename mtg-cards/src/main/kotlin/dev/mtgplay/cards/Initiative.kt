...
/**
 * Avenging Hunter — `{4}{G}` Creature — Elf Ranger, a 5/4. "Trample. When this creature enters, you
 * take the initiative." Elves' four-of payoff and the deck's only card that ventures.
 *
 * **Its printed type line is Elf Ranger, and the repo's Scryfall snapshot said Dragon Ranger.** The
 * snapshot is wrong and this is not a cosmetic disagreement: Elves plays four copies alongside Priest
 * of Titania, Timberwatch Elf and Wellwisher, every one of which counts Elves on the battlefield
 * (CR 205.3m), so a Dragon here would silently produce less mana, a smaller pump and less life. The
 * snapshot's `type_line` is corrected in the same packet; `W11`'s report records the correction.
 *
