 ...
 * and 7b, Nyxborn Hydra needs `{X}` and bestow (both built later, `FW-X` and `W10-C`), Writhing
 * Chrysalis needs two absent trigger
 ...
 * (`Targets.kt`). **Nyxborn Hydra** needed bestow (`FW-BESTOW`) and a CR 614.1c "enters with X +1/+1
 * counters" replacement; the counters framework it is sometimes filed under supplied neither, and the
 * variable-cost framework shipped without a pool card for that reason. `W10-C` built both and the card
 * is now [nyxbornHydra] (Bestow.kt) — it is the pool's first client of `{X}` on a spell.
 *
 * The `W10-C` packet adds the pool's one **Spacecraft** (Spacecraft.kt): [pinnacleKillShip], whose
 * Station ability is a cost with a chosen object ("Tap another creature you control"), a counter count
 * scaled by that creature's layered power, and a CR 604.3 static ability that makes the permanent an
 * artifact creature with flying at seven counters. It corrects two recorded diagnoses. `W8-G` filed the
 * card as needing a layer-4 type change *and* a layer-7b P/T setting: the P/T setting is not merely
 * unnecessary but wrong, because a Spacecraft's 7/7 is **printed on the card** (CR 208.1b) and what
 * blocked the card was `PrintedCharacteristics` asserting that only creature cards carry a P/T box. And
 * `FW-TYPECHANGE`'s note that a layer-4 change is always generated by a resolving ability was a claim
 * about the pool that this card disproves — nothing resolves when the seventh counter lands. The
 * conditional-continuous-effect question the packet brief raised turned out to be already answered:
 * `FW-CONDSTATIC`'s [dev.mtgplay.core.definition.StaticCondition] is re-evaluated on every
 * characteristic read, so a static effect's applicability tracks a counter count for free.
 *
 * That packet's other card is [nyxbornHydra] (Bestow.kt), the gauntlet's one **bestow** printing
 * (CR 702.103) and the last of the deferred ten. Three of its five recorded blockers turned out to be
 * already built — entering attached, `{X}` on an alternative cost, and "becomes a creature again when
 * the host leaves", which needs no rule of its own because the type change is *conditioned* on being
 * attached and stops applying the instant the host goes. The two that were real are the first printed
 * layer-4 type **removal** ("an Aura enchantment and not a creature") and the CR 614.1c enters-with-
 * counters replacement `ReplacementEffect` had listed as an extension point for four packets. The one
