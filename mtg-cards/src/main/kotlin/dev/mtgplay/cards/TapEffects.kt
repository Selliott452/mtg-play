 ...
 * Faeries' Harrier Strix and Snap, Elves' and Spy Combo's Quirion Ranger, Mono-Blue Terror's Sleep of
 * the Dead, and UWX Familiar's Azorius Chancery.
 *
 * Five cards, four new primitives, and the primitives are the point — until this packet the engine
 ...
 *   permanent already in the requested status rather than failing.
...
/** Harrier Strix, for the trigger that taps a permanent (CR 603). */
private val HARRIER_STRIX: CardRef = CardRef("Harrier Strix")
...
/** How many cards Harrier Strix's activated ability draws before its discard (CR 121.1). */
const val HARRIER_STRIX_DRAW: Int = 1

/** How many cards Harrier Strix's activated ability discards after its draw (CR 701.8). */
const val HARRIER_STRIX_DISCARD: Int = 1
...
/**
 * Harrier Strix — `{U}` Creature — Bird 1/1. "Flying. When this creature enters, tap target permanent.
 * {2}{U}: Draw a card, then discard a card."
 *
 * A Faeries tempo one-drop, and the **first client of [tapPermanent]**. Every one of its three halves
 * is published vocabulary once that primitive exists: [Keyword.FLYING] (CR 702.9), a targeted
 * enters-the-battlefield trigger (`FW-ABILTGT`), and a [DrawThenDiscard] clause on an activated ability
 * (`FW-CLAUSEHOOK`, the same clause Faithless Looting's resolution is).
 *
 * **"Target permanent", not "target creature"**, which is [PermanentRestriction.ANY_PERMANENT]: the
 * Strix taps a blocker, but it equally taps an untapped land in the opponent's upkeep to strand a
 * counterspell. Encoding it as `CREATURE` would delete a real line (PLAN.md §7).
 *
 * **A permanent that is already tapped is a legal target and simply stays tapped** (CR 701.21a). The
 * trigger neither checks nor fizzles for it — a mandatory targeted trigger with a legal target resolves
 * and does what it can — which is exactly why [tapPermanent] is a separate function from the `{T}`
 * cost's tap, whose whole job is to refuse a tapped source (CR 602.2a).
 *
 * The activated ability's [DrawThenDiscard] runs **after** its (empty) ordinary effect and pauses for
 * the discard, routing it through the CR 614/616 framework — so a madness card discarded to a Strix is
 * exiled instead, even though Faeries prints none.
 */
                ...
                TriggeredAbility(
                    ...
                    effect =
                        ResolutionEffect { state, context ->
                            tapPermanent(state, onePermanentTarget(context.targets, HARRIER_STRIX.name))
                        },
                ),
            )
        override val activatedAbilities =
            persistentListOf(
                ActivatedAbility(
                    ...
                    drawThenDiscard = DrawThenDiscard(HARRIER_STRIX_DRAW, HARRIER_STRIX_DISCARD),
                ),
            )
    }
