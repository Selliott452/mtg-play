...
class DealDamageToEachPermanentSpec :
    StringSpec({
        // `FW-PREVENT`: the sweeper names its source (CR 120.1). A sweeper's own source is routinely
        // one of its recipients — Krark-Clan Shaman damages every creature without flying, itself
        // included — and until the source was threaded the primitive could not say so, which left
        // CR 615 prevention and CR 702.16e protection inexpressible for exactly the shape that needs
        // them most. Colourless here (no mana cost, CR 105.4), so no quality matches it.
        val sweeperSource = DamageSource(objectId = null, card = CardRef("Bear"))
        ...
        "CR 120.3d: sweep damage is marked on every accepted creature and on no other permanent" {
            val swept = dealDamageToEachPermanent(board(), sweeperSource, 2, ::isCreaturePermanent)
        ...
        "CR 608.2: the predicate narrows the affected set — a controller qualifier spares the rest" {
            val swept =
                dealDamageToEachPermanent(board(), sweeperSource, 3) { state, obj ->
        ...
        "CR 608.2: the affected set is fixed from the state the effect began in" {
            ...
            val swept =
                dealDamageToEachPermanent(board(), sweeperSource, 1) { state, obj ->
        ...
        "CR 120.8: sweeping zero damage is not dealt at all — no marks, no events" {
            val start = board()
            val untouched = dealDamageToEachPermanent(start, sweeperSource, 0, ::isCreaturePermanent)
        ...
        "CR 120: a negative sweep amount is an error, never silently clamped" {
            shouldThrow<IllegalArgumentException> {
                dealDamageToEachPermanent(board(), sweeperSource, -1, ::isCreaturePermanent)
            }
        }

        "CR 120: a predicate that accepts nothing changes nothing" {
            val start = board()
            val untouched = dealDamageToEachPermanent(start, sweeperSource, 4) { _, _ -> false }
