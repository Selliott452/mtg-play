...
class LayerSystemSpec :
    StringSpec({
        ...
        "CR 303.4a: enchant creature — only creatures are legal targets, both directions" {
            ...
            isTargetLegal(state, spec, Target.Permanent(ObjectId(1)), alice) shouldBe false
        }
        ...
        "CR 303.4a: enchant land — only lands are legal targets, both directions" {
            ...
            legalTargets(state, spec, alice) shouldContainExactly listOf(Target.Permanent(ObjectId(1)))
            isTargetLegal(state, spec, Target.Permanent(ObjectId(0)), alice) shouldBe false
        }
        ...
        "CR 303.4a: enchant creature you control — control is ownership (§4), both directions" {
            ...
            legalTargets(state, spec, alice) shouldContainExactly listOf(Target.Permanent(ObjectId(0)))
            legalTargets(state, spec, bob) shouldContainExactly listOf(Target.Permanent(ObjectId(1)))
        }
