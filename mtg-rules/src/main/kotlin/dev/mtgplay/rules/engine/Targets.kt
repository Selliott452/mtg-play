...
internal fun legalTargets(
    ...
    when (spec) {
        ...
        -> battlefieldTargets(state, spec, you, chooser, context)
...
private fun battlefieldTargets(
    ...
    you: PlayerId,
    chooser: Chooser,
    // `W9-C`: a restriction may read what the choosing object has already settled (Searing Blaze's
    // second line names the first line's player), so the context travels with the candidate test.
    context: TargetContext = TargetContext.NONE,
): List<Target> =
    state.sharedZones.battlefield
        .filter { candidate ->
            val admitted =
                when (spec) {
                    is TargetSpec.TargetPermanent ->
                        satisfiesPermanentRestriction(state, spec.restriction, candidate, you, context)
                    is TargetSpec.Enchantable ->
