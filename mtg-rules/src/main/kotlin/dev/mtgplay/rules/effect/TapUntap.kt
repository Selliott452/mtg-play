 ...
 * matching event only when the status actually changes. A no-op for an id that is not on the
 * battlefield, and for one already in the requested status (CR 701.21a, CR 701.21b).
 *
...
private fun setTapped(
    ...
    val permanent: GameObject? = state.sharedZones.battlefield.firstOrNull { it.id == objectId }
    // Not on the battlefield, or already in the requested status: nothing happens and nothing is said.
    if (permanent == null || permanent.tapped == tapped) return state
 ...
 *
 * A no-op off the battlefield, for [setTapped]'s reason. The marker is consumed by
...
fun skipNextUntapStep(
    ...
    // Not on the battlefield, or already marked: the marker is a fact, not a counter.
    if (permanent == null || permanent.skipsNextUntapStep) return state
    val index = state.sharedZones.battlefield.indexOfFirst { it.id == objectId }
